...
def bix_upload_data_processados_bucket(df, path, s3_path, bucket, ambiente_execucao_EMR):
    ...
    if ambiente_execucao_EMR:
        ...
        # Grava no log
        bix_grava_log("Log bix - Este Script Executa Somente em Cluster EMR", bucket)

# Define uma função para carregar um modelo de machine learning para um bucket S3
def bix_upload_modelos_ml_bucket(model, path, s3_path, bucket, ambiente_execucao_EMR):
